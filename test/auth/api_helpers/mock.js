...
function mockAPI(steps) {
  ...
  steps.forEach(function (step) {
    ...
    switch (type) {
      case 'SwaggerClient':
        nock('http://0.0.0.0:13337').get('/api/authapi/authcore.swagger.json?v=0.2').times(count)
          .replyWithFile(
      ...
      case 'SwaggerClientForHTTPS':
        nock('https://0.0.0.0:13338').get('/api/authapi/authcore.swagger.json?v=0.2').times(count)
          .replyWithFile(
            200, `${__dirname}/../../authcore.swagger.json`,
            { 'Content-Type': 'application/json' }
          )
