...
class Client {
  ...
  async getCurrentUser () {
    const resp = await this._http(true).get(basePath + '/users/me')
    return resp.data
  }
  ...
  async listUserEvents (userId, pageToken, limit) {
    ...
    const resp = await this._http(true).get(`${basePath}/events`, { params })
    return resp.data
  }
  ...
  async listUserIDP (userId) {
    ...
    const resp = await this._http(true).get(`${basePath}/users/${userId}/idp_users`)
    return resp.data
  }
  ...
  /**
   * List Second Factor of a user specified by user ID.
   *
   * @param {number} userId The user's ID.
   * @returns {object} List of user's second factors.
   */
  async listUserSecondFactors (userId) {
    if (!typeChecker(userId, 'number', true)) {
      throw new Error('userId is required and has to be number format')
    }
    const resp = await this._http(true).get(`${basePath}/users/${userId}/second_factors`)
    return resp.data
  }
  ...
  async deleteCurrentSession () {
    await this._http(true).delete(basePath + '/sessions/me')
  }
